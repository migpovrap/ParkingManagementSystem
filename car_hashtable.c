 ...
 * by both stuctures, the search function for both and some auxilary ones.
 */
...
/**
 * @brief 
 * The hash function using the djb2 algorithm to calculate the hash of the license plate a string.
 * @param mt The license plate of the vehicle in question.
 * @param size The size of the hashtable (it is calculated based on the max capacity of the park).
 * @return (unsigned long) Returns the index for the hashtable that is correspondent
 * to that licence plate late.
 */
...
/**
 * @brief 
 * A function to search the hashtable that takes in account the separate chaining
 ...
 * @param mt The license plate of the vehicle in question.
 * @return Car* Returns the pointer to the car that was removed from the hashtable, this pointer needs to be saved somewhere (the logcars linkedlist) or freed.
 */
Car* remove_car_hashtable(int parknumber, ParksData* parksdata, char mt[9]) {
	...
	Car* current_car = parksdata->parks[parknumber].cars[i]; //Sets current car to the head of the linked list (separate chaining).
	Car* prev_car = NULL; 
	...
	while (current_car != NULL) {
		if (current_car == exit_car) {
			if (prev_car == NULL) {
				...
				prev_car->next = current_car->next; //If the car removed is not the head of the list, it is bypassed.
			}
